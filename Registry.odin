...
//* REGISTER Table(T)
component_register_table :: proc($T: typeid, registry: ^Component_Registry, name: string, table: ^ode.Table(T), flags: Component_Flags = {.Runtime}) -> Component_ID {
    assert(registry != nil)
    assert(table != nil)
    assert(name != "")
    // existing name
...
component_find_by_name :: proc(registry: ^Component_Registry, name: string) -> ^Component_Descriptor {
    if registry == nil do return nil
    id, ok := registry.by_name[name]
    if !ok do return nil
    return component_find(registry, id)
}
component_find_by_type :: proc($T: typeid, registry: ^Component_Registry) -> ^Component_Descriptor {
    if registry == nil do return nil
