...
// Private
    ...
    // Sentinel for `related` when no entity is related to the event (non-Parent_*/Pair_* events). Encoded as the i64 bit pattern {ix = DELETED_INDEX, gen = 0}; constructed via u32 cast because the Odin checker does not accept bit_field compound literals as compile-time constants.
    @(private)
    RELATED_NONE :: cast(entity_id) u32(0xFFFFFFFF)
    ...
    database__notify_observers :: #force_inline proc(
        ...
        related: entity_id = RELATED_NONE, data: rawptr = nil,
    ) {
